...
class StereoDepth(object):
    ...
    def task(self, frameL, frameR, waitKey=5):
        ...
        heightr = max(rectifiedL.shape[0], rectifiedR.shape[0])
        widthr = rectifiedL.shape[1] + rectifiedR.shape[1]
        img = np.zeros((heightr, widthr, 3), dtype=np.uint8)
        img[0:rectifiedL.shape[0], 0:rectifiedL.shape[1]] = rectifiedL
        img[0:rectifiedR.shape[0], rectifiedL.shape[1]:] = rectifiedR
        interval = 50
        color = (0, 255, 0)
        # 绘制等间距平行线
        for k in range(heightr // interval):
            cv2.line(img, (0, interval * (k + 1)), (2 * widthr, interval * (k + 1)), color, 2, lineType=cv2.LINE_AA)
        if 1:
